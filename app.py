...
#     def __init__(self):
#         self.selected_glasses = 0  # <-- KLJUČNO!
...
#             # Face shape
#             shape = detect_face_shape(lm, w, h)
#             self.face_shape = shape
#             self.recommendation = recommend_glasses(shape)
...
#     column.image(img[:, :, :3], width=120, caption=filename)

#     if column.button(f"Select {filename}", key=f"btn_{i}"):
...
# # <-- OVDE se ažurira izabran okvir za processor
